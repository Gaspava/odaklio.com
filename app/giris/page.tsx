...
export default function GirisPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (!phone.trim()) {
      setError("Telefon numarasını gir.");
      return;
    }
      ...
      <div className="auth-card-wrapper">
        <div className="auth-card">
          <div className="auth-card-header">
            <h1 className="auth-title">Tekrar Hos Geldin</h1>
            <p className="auth-subtitle">Hesabina giris yap ve ogrenmeye devam et</p>
          </div>
          ...
          <div className="auth-footer">
            <span style={{ color: "var(--text-tertiary)" }}>Hesabin yok mu?</span>
            <Link href="/kayit" className="auth-link">Uye Ol</Link>
          </div>
        </div>
      </div>
    </div>
...
function translateError(msg: string): string {
  if (msg.includes("Invalid login credentials")) return "E-posta veya sifre yanlis.";
  if (msg.includes("Email not confirmed")) return "E-posta adresin henuz dogrulanmadi. Gelen kutunu kontrol et.";
